...
if __name__=='__main__':
    ...
    # # Number of rounds (number of posterior updates to perform)
    # K = 20
    # # Number of gradient steps per round
    # T = 2500
    ...
    # optAlloc = getOptimalAllocation(X, Z, theta, initAlloc, eta=1e-2)
    
    # bandit = TransductiveBandit(arms=X, Z=Z, initAlloc=initAlloc,
    #                             nSteps=T, nRounds=K, lambd_reg=.2,
    #                             theta=theta,eta=1e-2)
    # bandit.play()
    
    # # optAlloc = getOptimalAllocation(initAlloc, Z, bandit.posteriorMean[-1], bandit.posteriorCovar[-1], eta=1e-2, n=10000)
    
    # print('Optimal sampling allocation computed by mirror descent:')
    # print('{}'.format(optAlloc))
    # print('')
    
    # print('Sampling allocation from bootstrapping algorithm:')
    # print('{}'.format(bandit.allocation[-1,0]))
    
    # # See if the allocation produced by the bootstrapping algo converges to the optimal allocation
    # # (in a KL divergence sense)
    # allocHist = bandit.allocation[:,0]
    # klHist = np.sum(allocHist * np.log(allocHist/optAlloc),axis=-1)
    # fig,ax = plt.subplots(1)
    # ax.plot(klHist)
    # ax.set_xlabel('Round of bootstrapping algorithm')
    ...
    for d in dVals:
        ...
        for rep in range(nReps):
            ...
            try:
                assert(np.all(bandit.optZ==0))
            except:
                print('Claimed best arm: {}'.format(bandit.optZ))
                print('Best arm: {}'.format(0))
                incorrectCount += 1

...
class ETCBandit:
    ...
    def estimateTheta(self):
        ...
        """
        # Get standard basis vectors for feature map
        sbv = np.eye(self.n)[self.history[:self.t-1],:]
        
        # Compute feature map
        feat = np.reshape(self.C[:self.t-1,None,:] * sbv[:,:,None],
                          (self.t-1,self.C.shape[1]*sbv.shape[1]))
...
if __name__=='__main__':
    ...
    bandit = ETCBandit(arms=X, horizon=T, theta=theta,
